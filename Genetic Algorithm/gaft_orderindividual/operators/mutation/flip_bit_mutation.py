...
class FlipBitMutation(Mutation):
    ...
    def mutate(self, individual, engine):
        ...
        gen_length = len(individual.actions[0])

        if do_mutation:
            if type(individual) is OrderIndividual:
                actions = individual.actions
                print(actions)
                n_intervals = len(actions)
                print(individual.chromsome,'***************************')
                for i, genome in enumerate(individual.chromsome):
                    a, b = individual.ranges[i]
                    tmp_action = actions[int(uniform(0, n_intervals))]
                    gen_index = i % gen_length
                    individual.chromsome[i] = a + tmp_action[gen_index]
                p = individual.chromsome
                print('#################################', p)
                individual.solution = {'1': [p[0], p[1]], '2': [p[2], p[3]], '3': [p[4], p[5]], '4': [p[6], p[7]], '5': [p[8], p[9]]}
