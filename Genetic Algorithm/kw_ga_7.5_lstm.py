from gaft_LSTM import GAEngine
from gaft_LSTM.components import BinaryIndividual, Population, DecimalIndividual, OrderIndividual
from gaft_LSTM.operators import RouletteWheelSelection, UniformCrossover, FlipBitMutation, ExponentialRankingSelection, LinearRankingSelection
import math
from random import random, uniform
from gaft_LSTM.plugin_interfaces.analysis import OnTheFlyAnalysis
...
class GeneticAlgorithm:
    ...
    def generate(self):
        ...
        import random
        # eps only for 1st generationancestors
        ...
        try:
            ...
            first_action = []
            for a1 in [i/10 for i in range(0,11,int(eps * 10))]:
                for a2 in [i/10 for i in range(0,11,int(eps * 10))]:
            ...
            def fitness(indv):
                ...
                if reward >= 200 and policy not in good_p:
                    good_p.append(policy)
                if reward <= 200 and policy not in bad_p:
                    bad_p.append(policy)
...
if __name__ == "__main__":
    ...
    eps = 0.2
    EvaluateChallengeSubmission(ChallengeSeqDecEnvironment, GeneticAlgorithm, f"kw_7.5_{time_stamp}_{eps}.csv")
