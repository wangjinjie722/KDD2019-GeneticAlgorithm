...
class Population(object):
        ...
        if indvs is None:
            for _ in range(self.size):
                indv = IndvType(ranges=self.indv_template.ranges,
                                ...
                                actions=self.indv_template.actions,
                                best_1=self.indv_template.best_1)
    ...
    def best_indv(self, fitness):
        ...
        candidate_tmp = self.individuals
        candidate_tmp = sorted(candidate_tmp,
                   key=lambda indv: all_fits[candidate_tmp.index(indv)])
        best1 = candidate_tmp[-1]
        best2 = candidate_tmp[-2]
        return [best1, best2]
