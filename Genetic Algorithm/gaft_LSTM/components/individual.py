...
class Actions(object):
    ''' Descriptor for individual actions.
    '''
    def __init__(self):
        self.__actions = []
    ...
    def __set__(self, obj, actions):
        actions_org = actions
        #if type(actions) in [tuple, list]:
            #actions = [actions]*len(obj.ranges)
        # Check.
        if type(actions) not in [tuple, list]:
            raise TypeError('precisions must be a list of numbers')
        #if len(actions) != len(obj.ranges):
         #   raise ValueError('Lengths of actions and ranges should be the same')
        for (a, b), action in zip(obj.ranges, actions_org):
           # print(action)
            for single_action in action:
                #for step in single_action:
                if single_action > (b - a):
                    msg = 'Invalid action {} in range ({}, {})'.format(single_action, a, b)
                    raise ValueError(msg)
...
class Best_1(object):
    ...
    def __get__(self, obj, owner):
        return self.__best_1

    def __set__(self, obj, best_1):
        best_1_org = best_1
        #if type(actions) in [tuple, list]:
            #actions = [actions]*len(obj.ranges)
        # Check.
        if type(best_1) not in [tuple, list]:
            raise TypeError('precisions must be a list of numbers')
        #if len(actions) != len(obj.ranges):
         #   raise ValueError('Lengths of actions and ranges should be the same')
        for (a, b), best in zip(obj.ranges, best_1_org):
           # print(action)
            for single_best in best:
                #for step in single_action:
                if single_best > (b - a):
                    msg = 'Invalid action {} in range ({}, {})'.format(single_best, a, b)
                    raise ValueError(msg)
        self.__best_1 = best_1
...
class IndividualBase(object):
    ...
    # Designed actions for special problem (provided by users).
    actions = Actions()

    # The best action in 1st year (provided by users).
    best1 = Best_1()

    def __init__(self, ranges, eps, actions, best_1):
        self.ranges = ranges
        self.eps = eps
        self.precisions = eps
    ...
    def init(self, chromsome=None, solution=None):
        ...
        if not any([chromsome, solution]):
            ...
            self.chromsome[0] = self.best_1[0]
            self.chromsome[1] = self.best_1[1]
    ...
    def clone(self):
        ...
        indv = self.__class__(deepcopy(self.ranges), eps=deepcopy(self.eps), 
                actions=deepcopy(self.actions), best_1=deepcopy(self.best_1))
